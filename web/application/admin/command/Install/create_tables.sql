...
-- 1. 批次表（batches）
CREATE TABLE IF NOT EXISTS `fa_batches` (
...
-- 2. 币种表（coins）
CREATE TABLE IF NOT EXISTS `fa_coins` (
    `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- 3. 行情数据缓存表（coin_quotes）
CREATE TABLE IF NOT EXISTS `fa_coin_quotes` (
    `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- 4. 固件版本表（firmware_versions）
CREATE TABLE IF NOT EXISTS `fa_firmware_versions` (
    `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `fa_system_configs` (
    `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- 6. K线数据缓存表（coin_ohlc_cache）
CREATE TABLE IF NOT EXISTS `fa_coin_ohlc_cache` (
    `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
    `coin_id` VARCHAR(50) NOT NULL,
...
-- 7. API限流日志表（rate_limit_logs）
CREATE TABLE IF NOT EXISTS `fa_rate_limit_logs` (
    `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- 插入默认系统配置
INSERT INTO `fa_system_configs` (`config_key`, `config_value`, `config_type`, `description`) VALUES
...
-- 插入一个测试批次（用于开发测试）
INSERT INTO `fa_batches` (`name`, `api_key`, `status`, `rate_limit`) VALUES
('测试批次', 'test_api_key_123456', 1, 100)
